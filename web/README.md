...
|---|---|---|
| POST | `/api/v1/chat/completions` | OpenAI-compatible proxy (streaming supported); uses whichever key the signed-in user has configured, trying Groq → Gemini → OpenAI in that order (free tiers first) |
